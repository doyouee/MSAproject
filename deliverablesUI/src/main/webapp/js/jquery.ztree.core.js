...
(function ($) {
  var settings = {},
    ...
    data = {
      ...
      // 파일, 디렉터리 ID
      nodeId: function (setting, node) {
        var t = setting.data.simpleData.idKey === "" ? "" : setting.data.simpleData.idKey;
        var rawId = "" + node[t];
        if (typeof setting.data.simpleData.idKey === "function") {
          return setting.data.simpleData.idKey.call(this, rawId, node);
        }
        return rawId;
      },
      // 파일, 디렉터리 PID
      nodePid: function (setting, node) {
        var t = setting.data.simpleData.pIdKey === "" ? "" : setting.data.simpleData.pIdKey;
        var rawPid = "" + node[t];
        if (typeof setting.data.simpleData.pIdKey === "function") {
          return setting.data.simpleData.pIdKey.call(this, rawPid, node);
        }
        return rawPid;
      },
      ...
      // 파일의 등급
      nodeGrade: function (setting, node) {
        var t = setting.data.key.grade === "" ? "3" : setting.data.key.grade;
        var rawGrade = "" + node[t];
        if (typeof setting.data.render.grade === "function") {
          return setting.data.render.grade.call(this, rawGrade, node);
        }
        return rawGrade;
      },
      nodeType: function (setting, node) {
        var t = setting.data.key.type === "" ? null : setting.data.key.type;
        var rawType = "" + node[t];
        if (typeof setting.data.render.type === "function") {
          return setting.data.render.type.call(this, rawType, node);
        }
        return rawType;
      },
      nodeDeliverablesId: function (setting, node) {
          var t = setting.data.key.deilverablesId === "" ? null : setting.data.key.deilverablesId;
          var rawType = "" + node[t];
          if (typeof setting.data.render.type === "function") {
            return setting.data.render.type.call(this, rawType, node);
          }
    ...
    view = {
      ...
      expandCollapseNode: function (setting, node, expandFlag, animateFlag, callback) {
        ...
        if (
          !node.open &&
          isParent &&
          (!$$(node, consts.id.UL, setting).get(0) || (children && children.length > 0 && !$$(children[0], setting).get(0)))
        ) {
      ...
      makeDOMNodeIcon: function (html, setting, node) {
        var nameStr = data.nodeName(setting, node),
          grade = data.nodeGrade(setting, node),
          type = data.nodeType(setting, node),
          deliverablesId = data.nodeDeliverablesId(setting, node),
        ...
        if (type === "dir") {
          html.push("'class='", consts.className.NAME);
        }
        if (type === "file") {
          html.push("'class='node_file");
        }
        html.push("'>", name, "</span > ", " <div class=' grade grade", grade, "' id='fileCircle'></div> ", " <p class='fileInfo_box' deliverablesId=", deliverablesId, "'></p> ");
      },
      makeDOMNodeLine: function (html, setting, node) {
        var deliverablesId = data.nodeDeliverablesId(setting, node);
        html.push(
          "<span id='",
          node.tId,
          consts.id.SWITCH,
      ...
      makeDOMNodeNameBefore: function (html, setting, node) {
        var title = data.nodeTitle(setting, node),
          ...
          name = data.nodeName(setting, node),
          deliverablesId = data.nodeDeliverablesId(setting, node),
          deliverablesTopId = data.nodePid(setting, node),
          fontStyle = [];
      ...
      makeNodeLineClass: function (setting, node) {
        var id = data.nodeId(setting, node),
          pid = data.nodePid(setting, node);
        var lineClass = [];
